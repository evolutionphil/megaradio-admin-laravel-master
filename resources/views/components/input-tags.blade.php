...
<div x-data @tags-update="console.log('tags updated', $event.detail.tags)" data-tags='["aaa","bbb"]'>
    <div x-data="tagSelect()" x-init="init('parentEl')" @click.away="clearSearch()" @keydown.escape="clearSearch()">
        <div @keydown.enter.prevent="addTag(textInput)" class="relative">
            <input {{ $disabled ? 'disabled' : '' }}  type="text" x-model="textInput" x-ref="textInput" @input="search($event.target.value)">
            <input x-ref="textInput" type="hidden" name="{{ $attributes->get('name') }}">
    ...
    function tagSelect() {
        return {
            ...
            fireTagsUpdateEvent() {
                ...
                document.getElementsByName('tags')[1].value = JSON.stringify(this.tags)
            },
            hasTag(tag) {
